...
Файл `notes_NN.md` — це **повноцінний академічний конспект** лекції, призначений як для самостійного вивчення студентами, так і для автоматичного заповнення панелі SLIDE_NOTES у `_NN.html`.
...
- Кількість розділів `## N.` у `notes_NN.md` **приблизно відповідає** кількості змістових слайдів у `_NN.html`
...
При генерації `SLIDE_NOTES` в `_NN.html`:
